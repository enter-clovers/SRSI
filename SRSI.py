...
BASIC_AHP_WEIGHTS = {
    ...
    "carbon pool variation factor": 1.1443 / 100,
    "soil organic carbon": 2.3023 / 100,
...
ENCODING_MAPPING = {
    ...
    "the capacity of irrigation and drainage": ["Fully satisfaction or satisfaction", "Satisfaction or basial satisfaction", "Basic satisfaction or dissatisfaction", "Unknowns"],
    "surface soil texture": ["Loam", "Clay", "Sandy soil", "Gravely soil", "Unknowns"],
    "carbon pool variation factor": [
    ...
    "soil bulk density": ["Moderate", "Slightly light ", "Slightly heavy", "Unknowns"],
}
...
def get_weights(df, selected_cols, is_aggregated, aggregated_pollution_cols):
    ...
    if agg_col_in_valid:
        ...
        print(
            f"   Final comprehensive weight：{final_w[agg_col_idx]:.8f}（占比={final_w[agg_col_idx] * 100:.4f}%）"
        )
    else:
...
def score_features(df, land_type, selected_cols, is_aggregated):
    ...
    valid_cols = [col for col in mapped_selected_cols if col in df.columns]
    print(
        f"\n📝 Effective column in the scoring process：{len(valid_cols)}列 | Including pollution status：{AGG_POLLUTION_COL in valid_cols}"
    )
    ...
    for idx, row in df_score.iterrows():
        ...
        for col in valid_cols:
            ...
            # 基础指标赋分
                ...
                elif col == "carbon pool variation factor":
                    score = (
                        80
                        if val == 0
                        else 69
...
def calculate_final_score(
    ...
    for col in df_score.columns:
        ...
        if col == agg_col_name:
            ...
            print(
                f"   -> Include in column：{col} | Original weight={total_pollution_weight:.6f} (污染物Sum)"
            )
